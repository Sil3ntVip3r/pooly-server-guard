...
SOURCE_LIBS=(core health drift discord update systemd)
OPTIONAL_LIBS=(balloon)
missing_lib=0
...
for lib in "${OPTIONAL_LIBS[@]}"; do
  ...
  if [[ -r "$file" ]]; then
    # shellcheck source=/dev/null
    source "$file" || echo "WARN: optional library could not be sourced: $file"
  fi
done

file="$LIB_DIR/main.sh"
if [[ ! -r "$file" ]]; then
  echo "FAIL: required library missing: $file"
  exit 1
fi
