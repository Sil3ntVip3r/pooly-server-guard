...
report_prune(){
  ...
  if [[ "$retention" =~ ^[0-9]+$ && "$retention" -gt 0 ]]; then
    while IFS= read -r -d '' f; do if rm -f -- "$f"; then deleted_age=$((deleted_age+1)); fi; done < <(find "$REPORT_DIR" -maxdepth 1 -xdev -type f \( -name 'pooly-server-guard-watch-*.txt' -o -name 'pooly-server-guard-*.txt' \) -mtime +"$retention" -print0 2>/dev/null)
  else
  ...
  if [[ "$max" =~ ^[0-9]+$ && "$max" -gt 0 ]]; then
    ...
    if [[ "$count" -gt "$max" ]]; then
      excess=$((count-max))
      while IFS= read -r -d '' entry; do f="${entry#* }"; if [[ -n "$f" && -f "$f" ]]; then if rm -f -- "$f"; then deleted_count=$((deleted_count+1)); fi; fi; done < <(find "$REPORT_DIR" -maxdepth 1 -xdev -type f \( -name 'pooly-server-guard-watch-*.txt' -o -name 'pooly-server-guard-*.txt' \) -printf '%T@ %p\0' 2>/dev/null | sort -z -n | head -z -n "$excess")
    fi
  else
...
memory_pressure_active(){
  ...
  if [[ "${swap_total:-0}" -gt 0 ]]; then swap_pct="$(awk -v t="$swap_total" -v f="$swap_free" 'BEGIN{printf "%.0f", ((t-f)*100)/t}')"; if [[ "$swap_pct" =~ ^[0-9]+$ && "$swap_pct" -ge "${POOLY_SWAP_WARN_PCT:-20}" ]]; then return 0; fi; fi
  return 1
}
...
memory_diagnostics(){
  ...
  echo "Reason: RAM or swap crossed a warning/fail threshold, so alpha3.5 captured process evidence."
...
load_diagnostics(){
  ...
  cpus="$(nproc 2>/dev/null || echo 1)"; load_per_cpu="$(awk -v l="${load1:-0}" -v c="${cpus:-1}" 'BEGIN{if(c>0) printf "%.2f", l/c; else print "0.00"}')"
  echo "Reason: load per CPU crossed a warning/fail threshold, so alpha3.5 captured CPU/process evidence."
  echo "Snapshot UTC: $(date -u)"; echo "Load averages: ${load1:-0} ${load5:-0} ${load15:-0}"; echo "CPU cores: ${cpus:-1}"; echo "Load per CPU: $load_per_cpu"
...
node_label=(node+' '+{'001':'Toronto','002':'Mumbai','003':'Tokyo2','004':'Frankfurt'}.get(node,'')).strip()
...
embed={'title':f"{emoji} Pooly {status} — Node {node_label}",'description':f"Host: `{host}`\nVersion: `v{version}` | Timer: `{timer}`",'color':colors.get(status,0x95A5A6),'timestamp':iso_now,'fields':[],'footer':{'text':'Pooly Server Guard'}}
if status=='PASS':
    embed['fields'] += [{'name':'Health','value':health(),'inline':False},{'name':'Checks','value':checks(),'inline':False}]
else:
...
discord_send_watch(){
  ...
  payload_file="$(mktemp)"
  if discord_watch_payload_file "$status" "$host" "$node" "$report" "$tmp" "$payload_file"; then discord_post_json_file "$payload_file"; else echo "DISCORD RESULT: FAIL_PAYLOAD"; fi
  rm -f "$payload_file"
}

node_id(){
  local h ip
  h="$(hostname 2>/dev/null || true)"; ip="$(hostname -I 2>/dev/null | tr ' ' '\n' | grep -E '^[0-9]+\.' | head -1 || true)"
  case "$h $ip" in *001*toronto*|*104.225.219.167*) echo 001 ;; *002*mumbai*|*209.182.232.151*) echo 002 ;; *003*tokyo2*|*63.250.52.172*) echo 003 ;; *004*frankfurt*|*208.87.129.34*) echo 004 ;; *) echo unknown ;; esac
}
...
key_fingerprints(){ for u in "${ADMIN_USERS[@]}" root; do local home file; [[ "$u" == root ]] && home=/root || home="$(getent passwd "$u" | cut -d: -f6)"; file="$home/.ssh/authorized_keys"; echo "[$u]"; if $SUDO test -f "$file"; then $SUDO awk 'NF && $1 !~ /^#/' "$file" | while read -r line; do printf '%s\n' "$line" | ssh-keygen -lf - 2>/dev/null || echo "BAD_KEY_LINE"; done; else echo "NO_FILE"; fi; done; }
version_info(){ load_env; health_defaults; section "POOLY SERVER GUARD VERSION"; echo "Installed script version: $VERSION"; echo "Configured install path:  $POOLY_INSTALL_PATH"; echo "Configured repo path:     $POOLY_REPO_DIR"; echo "Configured timer:         $POOLY_WATCH_ONCALENDAR"; echo "Configured lock file:     $POOLY_LOCK_FILE"; echo "Report pruning:          ${POOLY_REPORT_PRUNE_ENABLED:-1}, ${POOLY_REPORT_RETENTION_DAYS:-14} day(s), max ${POOLY_REPORT_MAX_FILES:-1000} file(s)"; echo "Memory diagnostics:      RAM >= ${POOLY_RAM_WARN_PCT:-85}% or swap >= ${POOLY_SWAP_WARN_PCT:-20}%"; echo "Load diagnostics:        load/CPU >= ${POOLY_LOAD_WARN_PER_CPU:-2}"; echo "Discord embeds:          enabled for watch alerts; suppress PASS=${POOLY_DISCORD_SUPPRESS_PASS:-1}"; if [[ -d "$POOLY_REPO_DIR/.git" ]]; then run_as_report_owner git -C "$POOLY_REPO_DIR" rev-parse --short HEAD 2>/dev/null | awk '{print "Repo HEAD:              "$0}' || true; [[ -f "$POOLY_REPO_DIR/VERSION" ]] && awk '{print "Repo VERSION:           "$0}' "$POOLY_REPO_DIR/VERSION" || true; else echo "Repo state:             missing"; fi; }
self_update(){ section "POOLY SERVER GUARD UPDATE CHECK"; load_env; health_defaults; echo "Running version: $VERSION"; echo "Auto update: ${POOLY_GUARD_AUTO_UPDATE:-1}"; echo "Repo dir: $POOLY_REPO_DIR"; echo "Install path: $POOLY_INSTALL_PATH"; echo "Git user: ${REPORT_OWNER:-$(id -un)}"; if [[ "${POOLY_GUARD_AUTO_UPDATE:-1}" != "1" ]]; then echo "SKIP: auto update disabled"; echo "UPDATE RESULT: PASS"; return 0; fi; if ! command -v git >/dev/null 2>&1; then echo "FAIL: git is not installed"; echo "UPDATE RESULT: FAIL"; return 1; fi; if [[ ! -d "$POOLY_REPO_DIR/.git" ]]; then echo "FAIL: repo missing at $POOLY_REPO_DIR"; echo "UPDATE RESULT: FAIL"; return 1; fi; local branch remote_ref before after repo_version installed_changed=0; branch="${POOLY_GUARD_AUTO_UPDATE_BRANCH:-main}"; remote_ref="origin/$branch"; before="$(run_as_report_owner git -C "$POOLY_REPO_DIR" rev-parse --short HEAD 2>/dev/null || echo unknown)"; echo "Local repo before: $before"; if ! run_as_report_owner git -C "$POOLY_REPO_DIR" fetch --quiet --all --prune; then echo "FAIL: git fetch failed"; echo "UPDATE RESULT: FAIL"; return 1; fi; if ! run_as_report_owner git -C "$POOLY_REPO_DIR" reset --hard "$remote_ref" >/dev/null; then echo "FAIL: git reset to $remote_ref failed"; echo "UPDATE RESULT: FAIL"; return 1; fi; after="$(run_as_report_owner git -C "$POOLY_REPO_DIR" rev-parse --short HEAD 2>/dev/null || echo unknown)"; repo_version="$(cat "$POOLY_REPO_DIR/VERSION" 2>/dev/null || echo unknown)"; echo "Local repo after:  $after"; echo "Latest repo version: $repo_version"; if [[ ! -x "$POOLY_INSTALL_PATH" ]] || ! cmp -s "$POOLY_REPO_DIR/pooly-server-guard.sh" "$POOLY_INSTALL_PATH"; then need_sudo; $SUDO install -m 755 "$POOLY_REPO_DIR/pooly-server-guard.sh" "$POOLY_INSTALL_PATH"; [[ ${EUID:-$(id -u)} -eq 0 ]] && chown "$REPORT_OWNER:$REPORT_OWNER" "$POOLY_INSTALL_PATH" 2>/dev/null || true; installed_changed=1; echo "UPDATED: installed script refreshed from repo"; else echo "PASS: installed script already matches repo"; fi; if [[ "$repo_version" != "$VERSION" ]]; then echo "INFO: this running process is v$VERSION; installed script is now repo v$repo_version"; echo "INFO: the next timer/manual run will execute the updated script."; fi; [[ "$installed_changed" == "1" ]] && echo "UPDATE ACTION: INSTALLED" || echo "UPDATE ACTION: NONE"; echo "UPDATE RESULT: PASS"; return 0; }
verify(){ section "POOLY SERVER GUARD VERIFY"; local failed=0 ports sshdT root_count groups u; ports="$(sshd_policy | awk '$1=="port"{print $2}' | xargs echo)"; sshdT="$(sshd_policy)"; echo "SSH ports: $ports"; [[ "$ports" == "$SSH_PORT" ]] && echo "PASS: SSH effective port is $SSH_PORT only" || { echo "FAIL: SSH effective port is not $SSH_PORT only"; failed=1; }; ss -H -ltn 2>/dev/null | awk '{print $4}' | grep -Eq ':(22)$' && { echo "FAIL: sshd port 22 listener found"; failed=1; } || echo "PASS: no sshd port 22 listener found"; ss -H -ltn 2>/dev/null | awk '{print $4}' | grep -Eq ":${SSH_PORT}$" && echo "PASS: sshd port $SSH_PORT listener found" || { echo "FAIL: sshd port $SSH_PORT listener missing"; failed=1; }; awk '$1=="permitrootlogin" && $2=="no"{found=1} END{exit found ? 0 : 1}' <<< "$sshdT" && echo "PASS: root SSH disabled" || { echo "FAIL: root SSH not disabled"; failed=1; }; awk '$1=="passwordauthentication" && $2=="no"{found=1} END{exit found ? 0 : 1}' <<< "$sshdT" && echo "PASS: password SSH disabled" || { echo "FAIL: password SSH not disabled"; failed=1; }; awk '$1=="kbdinteractiveauthentication" && $2=="no"{found=1} END{exit found ? 0 : 1}' <<< "$sshdT" && echo "PASS: keyboard-interactive SSH disabled" || { echo "FAIL: keyboard-interactive SSH not disabled"; failed=1; }; for u in "${ADMIN_USERS[@]}"; do awk -v user="$u" '$1=="allowusers"{for(i=2;i<=NF;i++) if($i==user) found=1} END{exit found ? 0 : 1}' <<< "$sshdT" && echo "PASS: $u allowed" || { echo "FAIL: $u missing from AllowUsers"; failed=1; }; done; root_count="$($SUDO sh -c 'test -f /root/.ssh/authorized_keys && awk "NF && \$1 !~ /^#/" /root/.ssh/authorized_keys | wc -l || echo 0' 2>/dev/null | tail -1)"; [[ "$root_count" == "0" ]] && echo "PASS: root authorized_keys empty" || { echo "FAIL: root authorized_keys has $root_count active key(s)"; failed=1; }; for u in "${ADMIN_USERS[@]}"; do groups="$(id -nG "$u" 2>/dev/null || true)"; [[ " $groups " == *" sudo "* ]] && echo "PASS: $u in sudo" || { echo "FAIL: $u not in sudo"; failed=1; }; done; $SUDO grep -RIs 'NOPASSWD' /etc/sudoers /etc/sudoers.d 2>/dev/null | grep -vE '^#|pooly-security-backups' >/dev/null && { echo "FAIL: active NOPASSWD sudo rule found"; failed=1; } || echo "PASS: no active NOPASSWD sudo rules"; systemctl is-active --quiet fail2ban && echo "PASS: Fail2Ban active" || { echo "FAIL: Fail2Ban inactive"; failed=1; }; echo; [[ $failed -eq 0 ]] && echo "RESULT: PASS" || echo "RESULT: FAIL"; return "$failed"; }
baseline_verify(){ section "POOLY BASELINE VERIFY"; local failed=0 swapgb filemax swappiness qdisc cc; grep -q '22.04' /etc/os-release && echo "PASS: Ubuntu 22.04" || { echo "FAIL: not Ubuntu 22.04"; failed=1; }; swapgb=$(free -g | awk '/Swap:/{print $2}'); [[ "${swapgb:-0}" -ge 19 ]] && echo "PASS: swap >= 20G" || { echo "FAIL: swap < 20G"; failed=1; }; filemax=$(sysctl -n fs.file-max 2>/dev/null || echo 0); [[ "$filemax" -ge 4194304 ]] && echo "PASS: fs.file-max >= 4194304" || { echo "FAIL: fs.file-max too low"; failed=1; }; swappiness=$(sysctl -n vm.swappiness 2>/dev/null || echo x); [[ "$swappiness" == 10 ]] && echo "PASS: vm.swappiness=10" || { echo "FAIL: vm.swappiness != 10"; failed=1; }; cc=$(sysctl -n net.ipv4.tcp_congestion_control 2>/dev/null || true); [[ "$cc" == bbr ]] && echo "PASS: TCP BBR" || { echo "FAIL: TCP BBR not active"; failed=1; }; qdisc=$(sysctl -n net.core.default_qdisc 2>/dev/null || true); [[ "$qdisc" == fq ]] && echo "PASS: fq qdisc" || { echo "FAIL: fq qdisc not active"; failed=1; }; systemctl is-active --quiet chrony && echo "PASS: chrony active" || { echo "FAIL: chrony inactive"; failed=1; }; command -v wg >/dev/null && echo "PASS: wireguard tools present" || { echo "FAIL: wireguard tools missing"; failed=1; }; command -v tcpdump >/dev/null && echo "PASS: tcpdump present" || { echo "FAIL: tcpdump missing"; failed=1; }; echo; [[ $failed -eq 0 ]] && echo "BASELINE RESULT: PASS" || echo "BASELINE RESULT: FAIL"; return "$failed"; }
...
watch_issue_action(){ local file="${1:?missing report tmp}"; if grep -Eq '^(DISK /|DISK WORST|JOURNAL SIZE:|GPTLOGS SIZE:).*(— WARN|— FAIL)' "$file"; then echo "Storage/logs crossed a threshold. Check disk, journal size, and GPTlogs before services are affected."; elif grep -Eq '^(RAM:|SWAP:).*(— WARN|— FAIL)' "$file"; then echo "Memory pressure crossed a threshold. Alpha3.5 captured top memory/process evidence in the report."; elif grep -Eq '^LOAD: .*(— WARN|— FAIL)' "$file"; then echo "CPU/load pressure crossed a threshold. Alpha3.5 captured top CPU/process evidence in the report."; elif grep -Eq '^REBOOT REQUIRED: yes' "$file"; then echo "Server reports reboot required. Plan a controlled reboot window when safe."; elif grep -Eq 'REPORT PRUNE RESULT: WARN|REPORT PRUNE RESULT: FAIL' "$file"; then echo "Report pruning needs attention. Review REPORT_DIR safety checks and GPTlogs report counts."; elif grep -Eq 'SERVICE HEALTH RESULT: FAIL|FAILED SERVICES RESULT: FAIL|SERVICE RESULT: FAIL' "$file"; then echo "A Pooly/system service check failed. Review service health and failed systemd units on this node."; elif grep -Eq 'SSHD DRIFT RESULT: FAIL|KEYS RESULT: FAIL|UFW DRIFT RESULT: FAIL|RESULT: FAIL' "$file"; then echo "A security or access-control check failed. Review SSH, keys, sudo, and firewall drift immediately."; else echo "Review the issue summary and open the report path on the affected server."; fi; }
guard_watch(){ clear_self_failed_state; mkdirs; load_env; health_defaults; local lock_rc=0; acquire_watch_lock || lock_rc=$?; if [[ "$lock_rc" == "75" ]]; then return 0; elif [[ "$lock_rc" != "0" ]]; then return "$lock_rc"; fi; local tmp failed=0 warned=0 host node report update_rc=0 outcome="PASS"; host="$(hostname)"; node="$(node_id)"; tmp="$(mktemp)"; { echo "LOCK RESULT: PASS"; self_update || update_rc=$?; report_prune || true; verify || true; baseline_verify || true; server_health || true; if memory_pressure_active; then memory_diagnostics || true; fi; if load_pressure_active; then load_diagnostics || true; fi; port_audit || true; keys_drift || true; sshd_drift || true; [[ "${POOLY_WATCH_WARN_UFW_DRIFT:-1}" == "1" ]] && ufw_drift || true; services_drift || true; service_health || true; failed_services_check || true; } | tee "$tmp"; if [[ "$update_rc" != "0" ]] || grep -Eq 'UPDATE RESULT: FAIL|RESULT: FAIL|DRIFT RESULT: FAIL|PORT RESULT: FAIL|KEYS RESULT: FAIL|SERVICE RESULT: FAIL|SERVICE HEALTH RESULT: FAIL|FAILED SERVICES RESULT: FAIL|SERVER HEALTH RESULT: FAIL|REPORT PRUNE RESULT: FAIL' "$tmp"; then failed=1; outcome="FAIL"; elif grep -Eq 'SERVER HEALTH RESULT: WARN|REPORT PRUNE RESULT: WARN|WARN:' "$tmp"; then warned=1; outcome="WARN"; fi; echo "WATCH RESULT: $outcome" | tee -a "$tmp"; report="$REPORT_DIR/pooly-server-guard-watch-$host-$(date -u +%Y%m%d-%H%M%S).txt"; cp "$tmp" "$report"; [[ ${EUID:-$(id -u)} -eq 0 ]] && chown "$REPORT_OWNER:$REPORT_OWNER" "$report" 2>/dev/null || true; if [[ $failed -ne 0 ]]; then discord_send_watch FAIL "$host" "$node" "$report" "$tmp" | tee -a "$tmp" "$report" >/dev/null || true; rm -f "$tmp"; return 1; fi; if [[ $warned -ne 0 ]]; then [[ "${POOLY_ALERT_ON_WARN:-1}" == "1" ]] && discord_send_watch WARN "$host" "$node" "$report" "$tmp" | tee -a "$tmp" "$report" >/dev/null || true; rm -f "$tmp"; return 0; fi; [[ "${POOLY_ALERT_ON_PASS:-0}" == "1" ]] && discord_send_watch PASS "$host" "$node" "$report" "$tmp" | tee -a "$tmp" "$report" >/dev/null || true; rm -f "$tmp"; return 0; }
