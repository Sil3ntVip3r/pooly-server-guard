...
version_info(){
  ...
  if declare -F balloon_defaults >/dev/null 2>&1; then
    balloon_defaults
    echo "Balloon monitoring:      enabled=${POOLY_BALLOON_MONITOR_ENABLED:-0}, event threshold=${POOLY_BALLOON_WARN_MIB:-1024} MiB"
  else
    echo "Balloon monitoring:      optional module unavailable"
  fi
...
balloon_watch_section(){
  local tmp rc=0 result
  if ! declare -F balloon_status >/dev/null 2>&1; then
    section "POOLY MEMORY BALLOON"
    if [[ "${POOLY_BALLOON_MONITOR_ENABLED:-0}" == "1" ]]; then
      echo "BALLOON SUPPORTED: unknown"
      echo "WARN: balloon monitor is enabled but the optional module is unavailable"
      echo "BALLOON STATE: MODULE_MISSING"
  ...
  tmp="$(mktemp)" || {
    section "POOLY MEMORY BALLOON"
    echo "BALLOON SUPPORTED: unknown"
    echo "WARN: unable to allocate isolated balloon output"
    echo "BALLOON STATE: ERROR"
    echo "BALLOON RESULT: WARN"
    return 0
  }

  ( balloon_status ) >"$tmp" 2>&1 || rc=$?
  cat "$tmp"
  result="$(awk -F': ' '/^BALLOON RESULT: (PASS|WARN|FAIL)$/{print $2; exit}' "$tmp")"
  if [[ -z "$result" ]]; then
...
balloon_command_missing(){
  section "POOLY MEMORY BALLOON"
  echo "WARN: optional balloon module is unavailable"
  echo "BALLOON STATE: MODULE_MISSING"
  echo "BALLOON RESULT: WARN"
  return 2
}
...
guard_watch(){
  ...
  { echo "LOCK RESULT: PASS"; self_update || update_rc=$?; report_prune || true; timer_status || true; verify || true; baseline_verify || true; server_health || true; balloon_watch_section || true; journal_growth || true; if memory_pressure_active; then memory_diagnostics || true; fi; if load_pressure_active; then load_diagnostics || true; fi; port_audit || true; keys_drift || true; sshd_drift || true; [[ "${POOLY_WATCH_WARN_UFW_DRIFT:-1}" == "1" ]] && ufw_drift || true; services_drift || true; service_health || true; failed_services_check || true; } | tee "$tmp"
  if [[ "$update_rc" != "0" ]] || grep -Eq 'UPDATE RESULT: FAIL|RESULT: FAIL|DRIFT RESULT: FAIL|PORT RESULT: FAIL|KEYS RESULT: FAIL|SERVICE RESULT: FAIL|SERVICE HEALTH RESULT: FAIL|FAILED SERVICES RESULT: FAIL|SERVER HEALTH RESULT: FAIL|BALLOON RESULT: FAIL|JOURNAL GROWTH RESULT: FAIL|REPORT PRUNE RESULT: FAIL' "$tmp"; then failed=1; outcome="FAIL"; elif grep -Eq 'SERVER HEALTH RESULT: WARN|BALLOON RESULT: WARN|JOURNAL GROWTH RESULT: WARN|REPORT PRUNE RESULT: WARN|WARN:' "$tmp"; then warned=1; outcome="WARN"; fi
...
Commands:
  ...
  balloon-status | balloon-history [lines]
